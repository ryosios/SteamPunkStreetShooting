...
public class CharacterAbilityAttack_WorldAttach : CharacterAbilityBase
{
    ...
    {
        if (context.WorldAttachPoint == null)
        {
            Debug.LogWarning("World attach point is not assigned.");
            return null;
        }
        ...
        var abilityTrans = Instantiate(_attackParticle.transform, context.WorldAttachPoint);
